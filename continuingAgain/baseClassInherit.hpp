//
//  baseClassInherit.hpp
//  
//
//  Created by Clifford Campo on 5/2/18.
//  Copyright © 2018 CliffordCampo. All rights reserved.
//
#ifndef __BaseClassInherit__
#define __BaseClassInherit__
...
#include "/usr/local/Cellar/gsl/2.5/include/gsl/gsl_cblas.h"
...
#include "/usr/local/pgsql95/pgsql090513/include/libpq-fe.h" //changed 2018-07-10T14:26:01
#define WONDERFUL 0
...
class BaseClassInherit    {
    friend CmdLineArgs;
private: //These values get set during execution time, not during compile time.
    char charConnectionString[80];
    const size_t szBaseClassInherit = sizeof(BaseClassInherit);
    ...
    const size_t szPrivateBaseClassInherit = sizeof(szBaseClassInherit) + sizeof(szPrivateBaseClassInherit) + sizeof(maxNumberOfAvgTemps) + sizeof(dbnameEqual) + sizeof(useridEqual) + sizeof(passwordEqual) + sizeof(hostidEqual) + sizeof(portidEqual) + sizeof(basicSELECT) + sizeof(lengthOfStaticPartsOfConnectionString) + sizeof(lengthOfDynamicPartsOfConnectionString) +sizeof(*CLA) + sizeof(szPublicPartsOfBaseClassInherit);
    const size_t szPublicPartsOfBaseClassInherit = szBaseClassInherit - szPrivateBaseClassInherit;
public:
    GetTimeOfDay* ptrGTOD; //2018-07-05T12:52:22 Added this pointer
    bool inMultiThreadMode;
    bool selectFromTableDoesPQFinish;
    BaseClassInherit *self;    //Set during hierarchical Construction
    ...
    float *ptrFloatCovarianceMatrix; //points to space, dynamically acquired in selectFromTable.cpp, to hold the covariance matrix with each element a 32-bit of type float.
    char *workCharString;
    ...
    int connectionResponsibilityCount; //INitialized to zero and increaments by 1 for every connection made to this Baseclass
    ...
    //Prototypes
    BaseClassInherit(const char = '0', const char * = "cjc", const char * = "LocalWeather", const char * = "5436", const char * = "127.0.0.1", const size_t = MAXNUMBEROFAVGTEMPS );
//    BaseClassInherit(char *, const char = '0', const char * = "cjc", const char * = "LocalWeather", const char * = "5436", const char * = "127.0.0.1", const size_t = MAXNUMBEROFAVGTEMPS );
