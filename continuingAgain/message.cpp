...
const char *sqlStat1 = " AS \"AvgIndependentVariable\", avg(%s) AS \"AvgDependentVariable\", stddev(%s) AS \"σOfDependentVariable\", min(%s) AS \"minDependentVariable\", max(%s) AS \"maxDependentVariable\", count(*) AS \"NumberOfDependentVariablesInThisBucket\" FROM tbl_energy_usage WHERE avtempf > ";  //¿¿Find out who needs this. Then either delete this ptrToCharArray or move this ptr to char array into myPrototypes, and don't forget to do the externs!!
...
//
//The driving need for the next set of SQL statement was the old Oasis basement dehumidifier, which in the summers of 2016 and 2017 (2016-05-15 to 2016-10-15 AND \
2017-05-15 to 2017-10-15, just ran and ran and consumend about 15kwh of eclectricity per day and did not remove one drop of water from the basement air. So the next \
set of SQL statements in the array of pointers to string contain after each FROM sub-selects to remove th data from these two mid-may to mid-October data spans. Each of \ the three DECLARE states is preceeded with a BEGIN SQL statement and is followed by a FETCH ALL IN myportal statement; this reduceds the number of times we need to \
request that PQexec execute some SQL command and thus should speed up the processing.
const char *newMotherOfAllSelectStatements[] = {"DECLARE myportal CURSOR FOR SELECT today.avtempf AS \"today_avtempf\", yesterday.avtempf AS \"yesterday_avtempf\", (today.avtempf-yesterday.avtempf) AS \"∆avtempf\", today.avtempk AS \"today_avtempk\", yesterday.avtempk AS \"yesterday_avtempk\", yesterday.date AS \"yesterday_date\", today.date AS \"today_date\", (today.date - yesterday.date) AS \"∆day\", yesterday.m1kwh, today.m1kwh, (today.m1kwh - yesterday.m1kwh) AS \"m1Usage\", yesterday.m2kwh, today.m2kwh, (today.m2kwh- yesterday.m2kwh) AS \"m2Usage\", (1 * (today.m2kwh - yesterday.m2kwh)  +1 * (today.m1kwh - yesterday.m1kwh))/(today.date - yesterday.date) AS \"AvgDaily_m1m2kwh_Usage\" \
    FROM \
    (SELECT date,  avtempf , avtempk, m1kwh, m2kwh, jd FROM tbl_energy_usage WHERE date NOT BETWEEN '2017-05-15' AND '2017-10-15' AND date NOT BETWEEN '2016-05-15' AND '2016-10-15' ORDER BY date asc) today \
    INNER JOIN \
    (SELECT date,  avtempf , avtempk, m1kwh, m2kwh, jd FROM tbl_energy_usage WHERE date NOT BETWEEN '2017-05-15' AND '2017-10-15' AND date NOT BETWEEN '2016-05-15' AND '2016-10-15' ORDER BY date asc) yesterday ON today.jd::int = 1 + yesterday.jd::int ORDER BY today.date;", /*Starting with the DECLARE …, above, this SQL does the m1m2kwh case */
