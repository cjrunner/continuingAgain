...
//  4) ptrbc: Pointer to the instance of the BaseClassInherit caller is using.
...
//
//  Assumptions: Using postgreSQL, C++, BaseClassInherit object found in file baseClassInherit.hpp
//
...
using namespace std;
//char *alignCenter(char, int, BaseClassInherit *, int, int, char *, int=ASSUMEDBUFFERSIZE);
...
Align::Align (void) {
    this->originalBufferLocation = nullptr;
    this->colWidthArray = nullptr;
 ...
 */
void Align::alignSetup( const char *headerRowSpacerPattern, int assumedBufferSize)  {
...
//Doing the code, below, saves us the tedius, and error prone, procedure of manually counting the number of characters between fields.
    if (this->originalBufferLocation == nullptr) {
        this->originalBufferLocation = new char[assumedBufferSize];
        this->workBufferToBeReturnedToCaller = this->originalBufferLocation;
    }
...
Align::~Align() {
    ;
    if (this->originalBufferLocation != nullptr) {
        delete [] this->originalBufferLocation;
        this->originalBufferLocation = nullptr;
    }
    if (this->colWidthArray != nullptr) {
        delete [] this->colWidthArray;
        this->colWidthArray = nullptr;
    }
    /*
